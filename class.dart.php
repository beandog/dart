...
// 			die;
		...
		function get_default_audio_streamid($audio, $method) {
			...
			switch($method) {
				...
				case 0: // First English track
				
					foreach($audio as $arr)
						if($arr['langcode'] == 'en')
							return $arr['streamid'];
				...
				case 1: // Best quality
					...
					foreach($audio as $arr) {
						...
						if($arr['langcode'] == "en" && ($arr['format'] == "ac3" || $arr['format'] == "dts")) {
					...
					foreach($arr_tracks as $arr) {
						if($arr['channels'] == $max_channels && ($dts == false || ($dts == true && $arr['format'] == 'dts'))) {
							return $arr['streamid'];
						}
					}
