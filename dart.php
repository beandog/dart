...
<?php
	...
	foreach($devices as $device) {
		...
		if($opt_info || $opt_encode_info || $opt_copy_info || $opt_rip_info || $opt_pts_info || $opt_import || $opt_archive || $opt_dump_iso) {
			if($debug)
		...
		if($access_device) {
			...
			// getting DVD info.
			if($opt_import || $opt_info || $opt_encode_info || $opt_copy_info || $opt_rip_info || $opt_pts_info) {
				if(!$batch_mode) {
					if($disc_indexed) {
						echo "* Imported:\tYes\n";
					} else {
